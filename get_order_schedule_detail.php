<?php
	...
	function ShowTable($preorder_date,$dd)
	{
		...
		$retstr .= '<tr style="background: lightblue; height: 22px; color: #fff;"><th>Item Name</th><th width="85">Quantity</th><th>Student EnrollNo</th><th>Student Name</th><th>Class</th><th>Category Type</th><th>Food Allergy</th>';
		
		$reportbol = new reportbol();
		$detail=$reportbol->get_order_scheduledetail_table_report($preorder_date);
		
		while ($list=$detail->getNext()) {
			$retstr .= '<tr><td style="font-weight:bold;">';
			$retstr .= htmlspecialchars($list['item_name']);
			$retstr .= '</td><td style="text-align:right;font-weight:bold;">';
			$retstr .= $list['total'];
			$retstr .= '</td><td>';
			$retstr .= $list['participant_enroll_no'];
			$retstr .= '</td><td style="font-weight:bold;">';
			...
			$retstr .= '</td><td>';
			if ( $list['others_allergy_food_description'] == '')
				$retstr .= $list['food_allergy'];
			else if ( $list['food_allergy'] == '')
				$retstr .= $list['others_allergy_food_description'];
			else
				$retstr .= $list['others_allergy_food_description'] . ' , ' . $list['food_allergy'];
			$retstr .= '</td></tr>';						
		}	
		$retstr .= '</table>';
		$retstr .= '</div>';	
