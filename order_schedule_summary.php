...
<script language="Javascript">
	...
	{		
		...
		sFilter= getFilter();
		oTable= jQuery('#order_summary_dtList').dataTable(
		{
			...
			"sAjaxSource": "get_order_schedule_summary.php",			
	...
	function export_summary_pdf()
	{
		...
		{
			var url ='export_summary_pdf.php?export_txtfromdate='+export_txtfromdate+"&export_txttodate="+export_txttodate;
			window.open(url);
		}
	}
	function export_summary_xls()
	{
		var export_txtfromdate = $('#txtfromdate').val();
		export_txtfromdate = $('#txtfromdate').val().replace('-','/');
		var export_txttodate = $('#txttodate').val();
		...
		if(export_txtfromdate=='')
			...
			$.get("export_summary_xls.php?export_txtfromdate="+export_txtfromdate+"&export_txttodate="+export_txttodate, check_export);
	}
	...
	{ 	
		if(data=='No Record')
			...
			window.location = "export_summary_xls.php?export="+1;
			return false;
		}
	}
</script>
...
<div class="content_data">
	<form id='frm_order' name='frm_order' method='POST'>
		...
		<h2>Order Schedule Summary Report</h2>
		...
		<table cellpadding="0"  border="1" class="display" id="order_summary_dtList" name="order_summary_dtList">
		...
		<div class="frm" style="text-align: right;">
			<input type="button" id="btnpdf" value="Download PDF" onclick="export_summary_pdf();"  class="btn" style="float: none;" />
			<input type="button" id="btnexcel" value="Download XLS" onclick="export_summary_xls();"  class="btn" style="float: none;" />
		</div>
	</form>
</div>
