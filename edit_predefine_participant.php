<?php
	...
	if (isset($_POST['btnedit'])) 	
	{
		...
		else if(trim($_POST['sel_student_gender'] == '-1'))
		{
			$errors[] = '* Please select student gender';
		}
		else if(trim($_POST['edt_sel_school_name'] == '-1'))
		{
			$errors[] = '* Please select school name';
		}
		...
		if(count($errors) == 0)
		{
			...
			$uploaded_file = $_POST['edt_hid_participant_id'].'/'.$_POST['hidden_upload_name'];
			...
			if($count_duplicate_updating==0)
			{
				...
				if($_FILES["upload_mc"]["size"] > 0){
					if(!is_dir("upload/".$_POST['edt_hid_participant_id'])){
						mkdir("upload/".$_POST['edt_hid_participant_id']);
					}
					...
					$uploaded_file = $_POST['edt_hid_participant_id'].'/'.$file_name.'_'.$currentTime.'.'.$file_type;
				...
				if($result){
					$_SESSION['student_msg'] = "Updated student successfully!.";
					...
					$_SESSION['student_msg'] = "Updated Fail";
				}
			}
			else
				$_SESSION['student_msg'] = "Duplicate Student ID";
...
<script type="text/javascript">			
	...
	function AddValidation()
	{
		...
		'rules':
			{		
				...
				'edt_sel_school_name':{'required':true},				
				'sel_student_gender':{'required':true}			
			},
		'messages':
			{
				...
				'edt_sel_school_name':{'required':"*Required School Name"},					
				'sel_student_gender':{'required':"*Required Student Gender Name"}					
			},				
	...
	function checkDuplicateStudentID(){
		var studentID = $("#edt_txt_student_id").val();
		var table = 'tbl_student_predefine';
		var field_name = 'predefine_participant_id';
		var id = $("#edt_hid_participant_id").val();
		var condition_arr = {'predefine_participant_enroll_no':studentID, 'predefine_participant_id!':id };
		var condition_str = JSON.stringify(condition_arr);
		jQuery.get("checkduplicate_predefine_participant.php?table=" + table+"&&field_name=" + field_name+"&&condition_str=" + condition_str
	...
	<form action="" method="POST" id="edit_student" name="edit_student" enctype="multipart/form-data">
		...
		<h2>Edit Student</h2>
			...
			<div class="frm">
				...
				<input type="text" name="edt_txt_student_id" id="edt_txt_student_id" value="<?php echo $predefine_participant_enroll_no; ?>" onblur="checkDuplicateStudentID()"/>
				<span style="color:red; margin-left:17%; display: none" id="my_dupilcate_error_show"></span>
			</div>
			...
			<div class="frm">
				...
				<input type="hidden" name="hidden_upload_name" value="<?php echo $upload_file_name; ?>">
