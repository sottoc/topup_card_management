<?php
	...
	{	
		...
		else if(trim($_POST['sel_school_name'] == '-1'))
		{
		...
		if(count($errors) == 0)
		{
			...
			$check_dup_saving_result=$student_predefinebol->check_duplicate_studentID($studentID);
			...
			if($count_duplicate_saving==0)
			{
				...
				if($_FILES["upload_mc"]["size"] > 0){
					if(!is_dir("upload/".$saved_predefined_student_id)){
						mkdir("upload/".$saved_predefined_student_id);
					}
					...
					$uploaded_file = $saved_predefined_student_id.'/'.$file_name.'_'.$currentTime.'.'.$file_type;
					move_uploaded_file($_FILES["upload_mc"]["tmp_name"], "upload/".$uploaded_file);
					$student_predefineinfo->set_upload_file($uploaded_file);
					$student_predefineinfo->set_predefine_participant_id($saved_predefined_student_id);
					$student_predefinebol->update_student_predefine_uploadfile($student_predefineinfo);
				}
				...
				if($saved_predefined_student_id)
					$_SESSION['student_msg'] = 'Saved student successfully!.';
				else
					$_SESSION['student_msg'] = 'Saving Fail';
			}
			else
		...
		'messages':
			{
				...
				'sel_school_name':{'required':"*Required School Name"},					
				'sel_student_gender':{'required':"*Required Student Gender"}					
			},				
	...
	<form action="" method="POST" id="addnew_student" name="addnew_student" enctype="multipart/form-data">
		<h2>Create Student </h2>
			...
			<div class="frm">
				...
				<input type="text" name="txt_student_id" id="txt_student_id" onblur="checkDuplicateStudentID()" value="" />
