...
function show_confirm_box(div_id,okFunc)
{
	$( "#"+div_id ).dialog({
	resizable: true,
	height: "auto",
...
/* Add new function for order schedule report checkbox */
 function message_popup(message,width)
{	
	$('body').append('<table id="divmsg" name="divmsg" style="display: none;"><tr><td>'+ message +'</td></tr></table>');		
	$('#divmsg').dialog ({
		autoOpen: true,
		resizable: false,
		draggable: true,
...
function nodata_message_popup(message,width)
	{	
		$('body').append('<table id="divmsg" name="divmsg" style="display: none;"><tr><td>'+ message +'</td></tr></table>');		
		$('#divmsg').dialog ({
			autoOpen: true,
		...
		$('#divmsg').dialog('widget').find(".ui-dialog-titlebar").hide();
	}
function confirm_popup(message,width)
{	
	$('body').append('<table id="divmsg" name="divmsg" style="display: none;"><tr><td>'+ message +'</td></tr></table>');		
	$('#divmsg').dialog ({
		autoOpen: true,
...
/* End Function */
function show_duplicate_error(result) {
	$('input[type="submit"]').removeAttr('disabled');
	$("#my_dupilcate_error_show").css('display','none');
	$("#my_dupilcate_error_show").html("");
	if(result > 0){
		$('input[type="submit"]').attr('disabled','disabled');
		$("#my_dupilcate_error_show").css('display','block');
		$("#my_dupilcate_error_show").html("Duplicate! Try another one.");
		return false;
	}
}