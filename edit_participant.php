<?php
	...
	if(isset($_GET['participant_id']) && $_GET['participant_id']!='')
	{
		...
		if($Rresult->rowCount()!= 0){
			...
			$old_participant_value_str = "upload_file=>".$upload_file.",predefine_participant_id=>".$predefine_participant_id.",participant_name=>".$participant_name.",participant_enroll_no=>".$participant_enroll_no.",others_allergy_food_description=>".$others_allergy_food_description.",participant_description=>".$participant_description.",participant_gender_id=>".$participant_gender_id.",organization_name=>".$organization_name.",allow_canteen_order=>".$allow_canteen_order.",gender_name=>".$gender_name.",participant_allergy_flag=>".$participant_allergy_flag."student_class=>".$student_class;
		}
	...
	{	
		...
		if(count($errors) == 0)
		{
			...
			if(isset($_POST['txt_class_name']) && $_POST['txt_class_name']!='' )
				$student_class = $_POST['txt_class_name'];
			...
			if($count_duplicate_saving==0)
			{
				...
				$participantinfo->set_student_class($student_class);
				...
				$participantinfo->set_upload_file($upload_file);
				...
				if($participant_id)
					$_SESSION['participant_message'] = "Child Updated successfully.";
				else
					$_SESSION['participant_message'] = "Failed in updating Child.";
			}
			else
...
<div class="content_data">
	<form action="" method="POST" id="addnew_participant" name="addnew_participant" enctype="multipart/form-data">
		<h2>Edit Child</h2>
			...
			?>
			<input type="hidden" name="hidden_predefine_participant_id" id="hidden_predefine_participant_id" value="<?php echo $predefine_participant_id; ?>">
			...
			<div class="frm">
				<div class="frm_labelcat">Child ID:</div>
			...
			<div class="frm">
				...
				<input type="text" id="txt_class_name" name="txt_class_name" value="<?php echo $student_class; ?>"/>
			</div>
...
<script type="text/javascript">
	...
	function show_enrolltbl() {
		if($("#txt_participant_name").val() == '' ){
			message_popup('Please fill student name.',350);
			$("#show_enroll_table").css("display",'none');
